...
ToolpoolParser — the public orchestrator.
Coordinates loading, merging, and analysis. Owns the singleton.

This file's only job:
  - Define ToolpoolParser (load, reload, watch, validate_references)
...
from toolpool.core.parsers.loader import load_all_included_files, read_yaml
from toolpool.core.parsers.merger import merge
from toolpool.core.parsers.orchestration import analyse
from toolpool.core.parsers.transformers import (
    parse_agent,
    parse_policy_engine,
    parse_server,
)
...
class ToolpoolParser:
    ...
    def __init__(self, config_path: str = "./toolpool.yml"):
        self.config_path = Path(config_path).resolve()
        self._manifest: Optional[ToolpoolManifest] = None
        self._reload_callbacks: list[Callable[[ToolpoolManifest], None]] = []

    # ── public API ────────────────────────────────────────────────────────────

    def load(self) -> ToolpoolManifest:
        if not self.config_path.exists():
            raise FileNotFoundError(
                f"toolpool.yml not found at: {self.config_path}\n"
                f"Create a toolpool.yml in your project root "
                f"or pass --config <path>"
            )
        raw = read_yaml(self.config_path)
        if not raw:
            raise ValueError("toolpool.yml is empty")
    ...
    def reload(self) -> ToolpoolManifest:
        old = self._manifest
        try:
            new = self.load()
            ...
            logger.error(f"Failed to reload toolpool.yml: {e}")
            if old is not None:
    ...
    def _assert_dict_format(self, raw: dict, key: str):
        ...
        style) and not a list (old toolpool format).

        Raises a clear ValueError pointing at the user's file — not at Toolpool.
        """
        value = raw.get(key)
        if value is None:
        ...
        if isinstance(value, list):
            raise ValueError(
                ...
                f"      my-{key[:-1]}:\n"
                f"        ...\n\n"
                f"  See the migration guide: https://docs.toolpool.dev/migration\n"
            )
    ...
    def _build(self, raw: dict) -> ToolpoolManifest:
        ...
        # instead of blaming Toolpool internals.
        ...
        metadata = ToolpoolMetadata(**raw.get("metadata", {"name": "Toolpool"}))
        ...
        manifest = ToolpoolManifest(
            toolpool=raw.get("toolpool", "0.1.0"),
            metadata=metadata,
...
def get_parser() -> ToolpoolParser:
    ...
    if _parser is None:
        raise RuntimeError(
            "ToolpoolParser not initialized — call init_parser() at startup"
        )
    return _parser
...
def init_parser_from_manifest(manifest: ToolpoolManifest) -> ToolpoolParser:
    ...
    Used by `toolpool discover`: discovery builds a manifest directly (no
