"""
toolpool/core/discovery/to_manifest.py

Bridge from autodiscovery results → ToolpoolManifest.

The existing parser (parser.py) builds manifests from toolpool.yml. This
module builds them from discovery output instead. Both produce the same
ToolpoolManifest shape so the API layer and UI don't care which path the
data came through.
...
def build_manifest(
    ...
    """
    Build a ToolpoolManifest from autodiscovery output.
