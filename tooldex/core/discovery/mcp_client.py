"""
tooldex/core/discovery/mcp_client.py
...
DEFAULT_CONCURRENCY = 16       # stdio: process-spawn bound
DEFAULT_HTTP_CONCURRENCY = 64  # HTTP/SSE: pure I/O, can go much higher
...
async def probe_all(
    ...
    """
    Probe many servers concurrently with separate pools for stdio vs HTTP/SSE.

    - stdio servers share `concurrency` slots (process-spawn bound).
    - HTTP/SSE servers share `http_concurrency` slots (pure I/O — safely higher).
    - `on_result(result)` is an optional async callback invoked as each result
      arrives, enabling SSE streaming without waiting for the full run.
    - Returned order matches the input list order.
    - One failure never stops the others.
    """
    if not servers:
        return []

    stdio_sem = asyncio.Semaphore(concurrency)
    ...
    async def _guarded(server: MCPServer) -> ToolDiscoveryResult:
        ...
        async with sem:
            result = await probe_server(server, timeout=timeout)
        if on_result is not None:
            await on_result(result)
        return result

    return list(await asyncio.gather(*[_guarded(s) for s in servers]))
